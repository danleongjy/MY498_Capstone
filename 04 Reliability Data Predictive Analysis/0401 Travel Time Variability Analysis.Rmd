...
## Data Prep
Cells in this section cannot be run without original databases
...
```
### Save out compiled data
```{r}
save(regressiondf, ttm_baseline_fastestoption, file = 'r_analysis_data.RData')
```

## Regression Analysis of RBTs
Cells in this section can be run with the provided .RData file

### Read in compiled data
```{r}
load('r_analysis_data.RData')
```
...
plot(unlist(regressiondf[,dv]), regressiondf$dv_local_mean, xlab = 'RBT', ylab = 'Local Mean RBT')
title(paste(c('Pearson Correlation between RBT and\nLocal Mean RBT:', round(cor(regressiondf[,dv], regressiondf$dv_local_mean), 2)), collapse = ''))
```
### Figure 16: Examine correlation between baseline and actual median travel times
```{r}
plot(regressiondf$traveltime, regressiondf$median_traveltime, xlab = 'Baseline Travel Time (min)', ylab = 'Observed Median Travel Time (min)')
lines(c(0,150),c(0,150), col = 'red')
title(paste('Pearson Correlation: ', round(cor(regressiondf$traveltime, regressiondf$median_traveltime), 3), collapse = ''))
```
...
```
### Figure 2: Visualise calculation of local mean for Stratford, E02000726
...
```
If neighbours' RBTs are high, then own RBT also tends to be high.  After controlling for this, increased reliance on bus, National Rail and Overground tend to increase the interval width, while tram tends to decrease the interval width.
...
```
```{r}
summary(results_lwr02$results_localrsqadj$local_rsq)
```
...
```
### Figure 13: Visualise local coefficient estimates
...
```
### Figure 19: Visualise local adjusted Rsq
```{r fig.width = 9, fig.height = 8}
tmap_mode('plot') # set to plotting mode
tm_shape(results_lwr02_localrsqadj_spatial) + 
  tm_fill('Local Rsq Adj', palette = 'Greens') +
tm_shape(londontube) + 
  tm_lines(lwd = .1, col = 'black') +
tm_shape(londonrail) + 
  tm_lines(lwd = .1, col = 'black') +
tm_layout(outer.margins = c(0,0,0,0), legend.position = c('left','bottom'), frame = F)
```
### Figure 20: visualise p-values for local coefficient estimates
```{r fig.width = 18, fig.height = 24}
tmap_mode('plot') # set to plotting mode
...
tm_shape(londonrail) + 
  tm_lines(lwd = .1, col = 'black') + 
tm_facets(ncol = 3, nrow = ceiling(ncol(results_lwr02_pvals_spatial) / 3), sync = T) + 
